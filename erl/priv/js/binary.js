    ...
    if(Blob.prototype.isPrototypeOf(evt.data) ) { //&& evt.data.length >= HEAD_SIZE) {
        ...
        reader.addEventListener("loadend", function() {
            ...
            if (head_char !== 132 || reader.result.byteLength != HEAD_SIZE) 
                  return { status: "error", desc: "not #binary{1}" };
            ...
            meta_reader.addEventListener("loadend", function() {
                if (typeof callback == 'function')
                     ...
                     console.log("Header fields { id: " + id + ", type: " + type + ", app: " + app + ", version: " + version + ", from: " + from + ", to: " + to + ", user1: " + user1 + ", user2: " + user2 + " }");
                }
            });
        ...
        return { status: "ok" };
    } else { return { status: "error", desc: "not #binary{2}" }; }
};
